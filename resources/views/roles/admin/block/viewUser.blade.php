                ...
                <div class="col-lg-10">
                    <div class="p-4">
                        ...
                        <form method="GET" action="{{ route('user-search') }}" class="d-flex flex-row mb-3">
                            <input class="form-control me-4 rounded-pill" type="text" name="search"
                                ...
                                <tbody>
                                    @forelse ($users as $user)
                                        <tr>
                                            ...
                                            <td>
                                                <form action="{{ route('toggle-blocked', $user->id) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button
                                                        class="btn btn-{{ $user->isBlocked == 0 ? 'primary' : 'danger' }}">
